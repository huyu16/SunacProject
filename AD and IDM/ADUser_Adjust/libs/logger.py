...
def errlog_comp(comperrmsg):
    # 自定义logger
    comperrlogger = logging.getLogger("comp_errlogger")
    # 自定义logger接收的日志级别
    comperrlogger.setLevel(logging.ERROR)
    # 判断列表是否已存在handle，如有不做创建
    if not comperrlogger.handlers:
        ...
        # 将日志格式绑定handle
        comperrhandle.setFormatter(comperrformat)
        # 将handle绑定logger
        comperrlogger.addHandler(comperrhandle)
        # 输出信息
    else:
        pass
    comperrlogger.error(comperrmsg)


def infolog_comp(compinfomsg):
    ...
    if not compinfologger.handlers:
        ...
        pass
    compinfologger.info(compinfomsg)
