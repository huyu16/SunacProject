...
init_list = [{'messageid':"ABCD",'messagetime':'9999-07-21T10:14:47.933Z','sourceip':'noattackip','attacktime':'9999-07-21T10:14:47'}]
file_path = '/acdata/script/netlog.json'
sourceip = 'noattackip'
attacktime = '9999-07-21T10:14:47'
    ...
    if dic_event['events']:
        ...
        for event in dic_event['events']:
            if event['event']['id'] not in list_messageid:
                ...
                altercontext = "attack 被攻击设备名：" + m_devname + " 被攻击IP：" + m_devip + " 攻击源IP：" + m_sourceip + " 攻击时间：" + m_time.strftime("%Y-%m-%d %H:%M:%S")
                cmdcontext = '/usr/bin/zabbix_sender -s "dcxservagg" -z 10.4.0.106 -k "huaweinetworklog" -o "' + altercontext + '"'
                #print(cmdcontext)
                os.system(cmdcontext)
                list_file_data.append({"messageid":event['event']['id'],"messagetime":event["event"]["timestamp"],"sourceip":m_sourceip,"attacktime":m_time.strftime("%Y-%m-%d %H:%M:%S")})
