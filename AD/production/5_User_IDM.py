...
from production.logger import infolog_user, errlog_user, debuglog_user
...
REC = re.compile('OU=融创集团,DC=SUNAC,DC=local')
...
try:
    ...
    while datetime.strptime(begindatestr, '%Y-%m-%d %H:%M:%S.%f') < CURRENT_DATE and DATE_LOOP == 'YES':
        ...
        NUM_LOOP = 'YES'
        # 最大获取900条的数据，循环9次
        while NUM <= 200 and NUM_LOOP == 'YES':
    ...
    errlog_user(e)
    errlog_user('IDM信息--异常的用户:%s' % (userid))
