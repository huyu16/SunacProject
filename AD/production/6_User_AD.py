...
import pymysql
from production.logger import infolog_user, errlog_user
...
try:
    ...
    dbcursor.execute('select userid,userorg from dic_idm_user where userhandled > 9 '
                     'and userorg like "%%OU=北京区域集团,OU=融创集团,DC=SUNAC,DC=local" ')
    processed_user = dbcursor.fetchall()
    for userobj in processed_user:

        adquery = adconn.search(search_base='ou=融创集团,dc=SUNAC,dc=local',
